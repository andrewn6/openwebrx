...
function setSquelchDefault() {
    e("openwebrx-panel-squelch").value = 0;
}
...
function demod_envelope_draw(range, from, to, color, line) { //                                               ____
    ...
    if (!(to_px < 0 || from_px > window.innerWidth)) // out of screen?
    {
        ...
        drag_ranges.ending = {
            x1: to_px - env_bounding_line_w - env_att_w,
            x2: to_px
        };
        drag_ranges.whole_envelope = {
            x1: from_px,
            x2: to_px
        };
    ...
    if (typeof line != "undefined") // out of screen?
    {
        ...
        if (!(line_px < 0 || line_px > window.innerWidth)) {
            drag_ranges.line = {
                x1: line_px - env_line_click_area / 2,
...
function add_problem(what) {
    ...
    for (var i = 0; i < problems_span.children.length; i++)
        if (problems_span.children[i].innerHTML == what) return;
...
function audio_flush() {
    ...
    if (we_have_more_than(audio_buffer_maximal_length_sec))
        while (we_have_more_than(audio_buffer_decrease_to_on_overrun_sec)) {
...
function waterfall_init() {
    ...
    waterfall_timer = window.setInterval(() => {
        waterfall_dequeue();
        secondary_demod_waterfall_dequeue();
    }, 900 / fft_fps);
    resize_waterfall_container(false); /* then */
    resize_canvases();
    scale_setup();
    mkzoomlevels();
...
function secondary_demod_waterfall_add(data) {
    ...
    secondary_demod_canvases.map((x) => {
        x.openwebrx_top += 1;
    });
    secondary_demod_canvases_update_top();
